...
<body>
	...
	<div id="wrapper">
		...
		<nav class="navbar navbar-default navbar-static-top" role="navigation"
			...
			<div class="navbar-header">
				<span class="navbar-brand"><a class="back"
						href="${pageContext.request.contextPath }/toMain.action">&lt;</a> Hello,${uname}!</span>
			</div>
			<!-- 导航栏左侧end -->
				...
				<div class="sidebar-nav navbar-collapse">
					<ul class="nav" id="side-menu">
						<li><a
							...
							class="active side_a_center"><i class="fa fa-edit fa-fw l"></i>当前借阅</a></li>
						<li><a
						...
						<li><a
							...
							class="side_a_center"><i class="fa fa-slack"></i>&nbsp;罚款记录</a></li>
						<li><a
				...
				<div class="col-lg-12">
								...
								<c:forEach items="${pagemsg.lists}" var="row">
									<tr>
										...
										<td><a href="#" class="btn btn-primary btn-xs"
											...
											onclick="again(${row.borrow_id})">续借</a> <a href="#"
											class="btn btn-danger btn-xs"
											onclick="lost({borrow_id:'${row.borrow_id}',bid:'${row.bid}',sid:'${row.sid}'})">挂失</a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
					...
					<div id="pagetable">
						<span class="page_center"> <c:if
								...
								<a
									href="${pageContext.request.contextPath }/login.action?sid=${sid}&spwd=${spwd}&currentPage=1">[首页]</a>
								<a
									href="${pageContext.request.contextPath }/login.action?currentPage=${pagemsg.currPage-1}">[上一页]</a>
							</c:if> <c:if test="${pagemsg.currPage != pagemsg.totalPage}">
								<a
									href="${pageContext.request.contextPath }/login.action?currentPage=${pagemsg.currPage+1}">[下一页]</a>
								<a
									href="${pageContext.request.contextPath }/login.action?currentPage=${pagemsg.totalPage}">[尾页]</a>
							</c:if>
		...
		function again(borrow_id) {
			$.post("${pageContext.request.contextPath}/again.action", {"borrow_id" : borrow_id}, function(data) {
				if (data == "OK") {
					alert("续借成功！");
		...
		function lost(parm) {
			$.post("${pageContext.request.contextPath}/lostBook.action", {
				"borrow_id" : parm.borrow_id,
				"bid" : parm.bid,
				"sid" :parm.sid
			}, function(data) {
				if (data == "OK") {
