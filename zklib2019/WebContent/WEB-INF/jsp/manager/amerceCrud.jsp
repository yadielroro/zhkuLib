...
<body>
		...
		<div class="panel panel-default">
			<div class="panel-body">
				<form class="form-inline" method="get"
					action="${pageContext.request.contextPath }/getAmerceBySid.action">
			...
			<div class="col-lg-12">
							...
							<c:forEach items="${pagemsg.lists}" var="row">
								<tr>
									...
									<td><a href="#" class="btn btn-primary btn-xs"
										...
										onclick="pay(${row.amerce_id})">缴纳</a></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
				...
				<div id="pagetable">
					<span> <c:if test="${pagemsg.currPage != 1}">
							<a
								href="${pageContext.request.contextPath }/getAmerceBySid.action?currentPage=1">[首页]</a>
							<a
								href="${pageContext.request.contextPath }/getAmerceBySid.action?currentPage=${pagemsg.currPage-1}">[上一页]</a>
						</c:if> <c:if test="${pagemsg.currPage != pagemsg.totalPage}">
							<a
								href="${pageContext.request.contextPath }/getAmerceBySid.action?currentPage=${pagemsg.currPage+1}">[下一页]</a>
							<a
								href="${pageContext.request.contextPath }/getAmerceBySid.action?currentPage=${pagemsg.totalPage}">[尾页]</a>
						</c:if>
	...
	// 删除客户
	function pay(amerce_id) {
		console.log(typeof mid);
	    if(confirm('该生确实支付罚款了吗?')) {
	$.post("${pageContext.request.contextPath}/pay.action",{"amerce_id":amerce_id},
	function(data){
	            if(data =="OK"){
	                alert("支付确认！");
	                window.location.reload();
